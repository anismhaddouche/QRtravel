...
router.post('/', async (req, res) => {
  try {
    ...
    // Generate a unique referenceCode server-side. We pull existing
    // codes for this agency to keep generation cheap and unique; the
    // 23505 catch below handles the (rare) race with a concurrent
    // insert by retrying once.
    const existingRows = await all(
      'SELECT "referenceCode" FROM travelers WHERE "agencyId" = $1',
      [trip.agencyId]
    );
    const usedRefs = new Set(existingRows.map(r => r.referenceCode));
    let referenceCode = generateUniqueRefCode(usedRefs);
    ...
    // 23505 = unique violation on the auto-generated referenceCode.
    // This is a race against a concurrent insert; surface a generic 500
    // (retry would be safe but we don't bother given how rare this is).
    if (err && err.code === '23505') {
      return res.status(500).json({ error: 'Erreur de génération — réessayez', code: 'REF_GENERATION_RACE' });
    }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          if (e && e.code === '23505') msg = 'Conflit interne — réessayez';
